...
export interface PblNgridMetaRowSetState<T extends PblNgridMetaColumnState | PblNgridGroupColumnState> extends PblNgridMetaRowState, PickPNP<PblColumnSet<T extends PblNgridMetaColumnState ? PblMetaColumnDefinition : PblColumnGroupDefinition>, never, 'rowIndex'> {
    cols: T[];
}
export declare type ColRowDefsToState<T> = T extends PblMetaRowDefinitions ? PblNgridMetaRowState : T extends PblColumnDefinition[] ? PblNgridColumnState[] : T extends PblColumnSet<PblMetaColumnDefinition>[] ? PblNgridMetaRowSetState<PblNgridMetaColumnState>[] : T extends PblColumnSet<PblColumnGroupDefinition>[] ? PblNgridMetaRowSetState<PblNgridGroupColumnState>[] : never;
export declare type BaseColumnDefinitionSetState<T = PblNgridColumnDefinitionSet> = {
    [P in keyof T]: T[P] extends PblNgridColumnDefinitionSet['table'] ? BaseColumnDefinitionSetState<T[P]> : ColRowDefsToState<T[P]>;
};
